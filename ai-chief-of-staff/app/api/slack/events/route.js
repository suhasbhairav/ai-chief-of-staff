...
[[ACTION: {"type": "add", "task": {"title": "Task description", "priority": "P1", "source": "Slack", "status": "Needs you", "dueDate": "Today"}}]]`;
        ...
        if (!llmConfigured) {
          agentReply = "LLM provider is missing. Add `LLM_PROVIDER`, `LLM_MODEL`, and `LLM_API_KEY` to your deployment environment to activate Aegis.";
        } else {
          try {
            const safeSlackDm = assertSafeDirectUserInput(event.text, "Slack direct message");
            const resObj = await guardedResponsesCreate(null, {
            ...
            console.error("LLM DM reply handler crashed:", llmErr);
            agentReply = `Sorry Richard, I ran into an error reviewing that: ${llmErr.message}`;
          }
        }
...
Return JSON strictly matching this schema:
{
  ...
  "assignee": "Name of task owner (e.g., Gavin Belson, Dinesh, Gilfoyle, Richard)"
            ...
            const completion = await guardedResponsesCreate(null, {
