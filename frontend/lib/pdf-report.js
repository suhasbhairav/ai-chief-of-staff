...
const addPageNumber = (doc) => {
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    doc.text("AI Chief of Staff", 40, height - 22);
...
const drawCover = (doc, report) => {
  ...
  doc.text("AI CHIEF OF STAFF", 40, 78);
...
const drawMethodology = (doc, report) => {
  ...
  doc.setFontSize(14);
  doc.setTextColor(...BRAND.ink);
  doc.text("Created by Suhas Bhairav", 40, y);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.setTextColor(...BRAND.muted);
  doc.text("https://suhasbhairav.com", 40, y + 18);
};
...
export const generateBoardMemoPdf = (memo) => {
  ...
  doc.text("Prepared by Suhas Bhairav | https://suhasbhairav.com", 40, y);
  y += 30;
